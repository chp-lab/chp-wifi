...
void mqtt_connect()
{
  ...
  Serial.println("Turn off buildin led to be signaling.");
  is_fact_reset(100);
  ...
  while(!mqtt.connect(client_name.c_str(), mqtt_username.c_str(), mqtt_password.c_str()))
  {
	if(WiFi.status() == WL_DISCONNECTED) {
  	...
  	get_wifi_rssi();
	Serial.println("Turn off buildin led.");
  	digitalWrite(build_in_led, LOW);
  	set_mqtt_flag(false);
  	chp_wifi_handle();
    ...
    delay(500);
	Serial.println("Turn on buildin led.");
...
void giantOta()
{
    Serial.println("Starting ota server name..." + String(get_ap_name()));
    ArduinoOTA.setHostname(get_ap_name().c_str());
...
//  Serial.println(wifi_reset_presed);
...
//    reset_wifi_con();
  ...
  if(kb_reset_pressed == 0) {
    ...
    delay(5000);
    kb_reset_pressed = digitalRead(SW1);
    if(kb_reset_pressed == 0) {
      Serial.println("WiFi Reset pressed!");
      reset_wifi_con();
    }
